...
const runProcess = input => (data, index = 0) => {
	...
	if (opcode === OP_CODES.ADD || opcode === OP_CODES.MULTIPLY) {		
		const [mode1, mode2/*, modeAddress*/] = [opAndModes.length > 2 ? opAndModes.substr(-3, 1) : "0", opAndModes.length > 3 ? opAndModes.substr(-4, 1) : "0"/*, opAndModes.length > 4 ? opAndModes.substr(-5, 1) : 0*/];
		...
		const mode1 = opAndModes.length > 2 ? opAndModes.substr(-3, 1) : "0";
